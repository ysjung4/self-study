...
class MusicApp extends Component {
    ...
    componentDidMount() {
        this.setState({
            filterData : this.state.musicData
        })
    }
    handleOpen = (id) => {
        ...
        this.setState({
            ...
            findData: musicData.find( music => music.rank === id )
        })
    ...
    render() {
        const { musicData, filterData, done, findData} =this.state
        return (
            <div className="MusicApp">
                <MusicForm onTitle = {this.handleTitle}/>
                <MusicList onOpen={this.handleOpen} filterData={filterData}/>
                <Modal done={done} findData={findData} onClose={this.handleClose}/>
            </div>
        );
    }
}
